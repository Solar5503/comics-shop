...
interface ICartButtonProps extends ComponentProps<'button'> {
  ...
  addToCartClickHandler: (e: React.MouseEvent) => void
}
const CartButton = ({
  className,
  isAddingToCart,
  ...
  const classes = `${styles['cart-button']} ${className}`

  return (
    <button
      className={classes}
      aria-label={isAddingToCart ? 'Remove from cart' : 'Add to cart'}
      type="button"
      name="Add to cart"
